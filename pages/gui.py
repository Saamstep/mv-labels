...
def main_page():
    with ui.header().classes('items-center bg-blue-100'):
        ...
        ui.space()
        logout_button = ui.button('Logout', icon='logout').props('flat')
        logout_button.bind_visibility_from(app.storage.user, 'authenticated')
        logout_button.on_click(lambda: app.storage.user.update(authenticated=False))
        logout_button.on_click(lambda: ui.navigate.to('/'))
...
if __name__ in {'__main__', '__mp_main__'}:
    ui.run(storage_secret='hello-world', title='Custom Sub Pages Example', dark=True, reload=True)
